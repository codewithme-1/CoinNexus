...
async function initAuthGate() {
    ...
    try {
        ...
        if (result.success) {
            // Capture dynamic wallets from backend and store them locally
            if (result.wallets) {
                sessionStorage.setItem("nexus_live_wallets", JSON.stringify(result.wallets));
            }
...
// --- DYNAMIC WALLET ADDRESS CONFIGURATION (Fallback) ---
const FALLBACK_WALLETS = {
    USDTTRC20: {
        address: "TTTQG8JByFQ3zyrd671X9Y3YRU6muiQdSx",
        label: "USDT (TRC20)"
    },
    BTC: {
...
async function handleCryptoDeposit(e) {
    ...
    // Attempt to pull the live gateway addresses generated by the admin from memory
    let activeWallets = FALLBACK_WALLETS;
    const storedWallets = sessionStorage.getItem("nexus_live_wallets");
    
    if (storedWallets) {
        try {
            activeWallets = JSON.parse(storedWallets);
        } catch(err) {
            console.warn("Could not parse dynamic wallets, using fallbacks.");
        }
    }

    const selectedWallet = activeWallets[network] || activeWallets.USDTTRC20;
